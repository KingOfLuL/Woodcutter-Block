package com.gmail.pascald701.WoodcutterMod.screen;
	...
	private static final Identifier TEXTURE = new Identifier("textures/gui/container/stonecutter.png");
	   ...
	   protected void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
	      ...
	      if (this.canCraft) {
	         ...
	         List<WoodcuttingRecipe> list = ((WoodcutterScreenHandler)this.handler).getAvailableRecipes();
	         ...
	         for(int l = this.scrollOffset; l < k && l < ((WoodcutterScreenHandler)this.handler).getAvailableRecipeCount(); ++l) {
	            ...
	            if (x >= n && x < n + 16 && y >= o && y < o + 18) {
	               this.renderTooltip(matrices, ((WoodcuttingRecipe)list.get(l)).getOutput(), x, y);
	            }
	         }
	      }
	   ...
	   private void renderRecipeIcons(int x, int y, int scrollOffset) {
	      ...
	      for(int i = this.scrollOffset; i < scrollOffset && i < ((WoodcutterScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
	         ...
	         int l = j / 4;
	         int m = y + l * 18 + 2;
	         this.client.getItemRenderer().renderInGuiWithOverrides(((WoodcuttingRecipe)list.get(i)).getOutput(), k, m);
	      }
